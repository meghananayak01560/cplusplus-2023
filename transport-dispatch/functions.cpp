...
LinkedList::LinkedList(const LinkedList& object) : headptr(nullptr) {
    ...
    // Copy first node
    node* origCurrent = object.headptr;
    headptr = new node(origCurrent->getvalue());
    
    // Copy rest of the nodes
    node* newCurrent = headptr;
    origCurrent = origCurrent->getptr();
    
    while (origCurrent != nullptr) {
        newCurrent->setptr(new node(origCurrent->getvalue()));
        newCurrent = newCurrent->getptr();
        origCurrent = origCurrent->getptr();
    }
}
...
int LinkedList::find_closest_less_than(int value) {
    ...
    while (current != nullptr) {
        ...
        // Update closest if current value is less than target AND larger than previous closest
