...
void print_inst_1() {
  cout << "*** enter operation: push <location> | insert <location> | pop | "
          "clear | print | locations | quit"
       << endl;
  cout << "*** enter location from above list on next line" << endl;
  cout << "*** enter an empty line to end input" << endl;
  cout << "***" << endl;
  cout << "*** WARNING: push operation pushes location in push order without "
          "any location ordering"
       << endl;
  cout << "***" << endl;
}

void print_inst_2() {
  cout << "*** Locations: Boston | Hartford | NewYork | Philadelphia | "
          "Baltimore | WashingtonDC | Pittsburgh | Cleveland | Detroit | "
          "Chicago | "
       << endl;
  cout << "*** Indianapolis | Nashville | Atlanta | Houston | Dallas | Austin "
          "| Denver | Albuquerque | Phoenix | Sacramento"
       << endl;
  cout << "*** .... you can repeat this list anytime at prompt by typing "
          "locations ...."
       << endl;
  cout << "***" << endl;
}
...
node::node(int arg_val, node *ptr) {
    value = arg_val;
    this->ptr = ptr;
}
...
void node::setptr(node *ptr) { this->ptr = ptr; }
...
LinkedList::LinkedList(const LinkedList &object) : headptr(nullptr) {
  ...
  // Create a temporary array to store values in order
  ...
  for (size_t i = 0; i < values.size(); i++) {
    if (headptr == nullptr) {
      ...
      insertNode(values[i], find_closest_less_than(values[i]));
    }
  }
}
...
int LinkedList::pop() {
  ...
  int value_remove = headptr->getvalue();
  node *temp = headptr;
...
void LinkedList::insertNode(int value, int reference_value) {
  if (link_list_empty() || reference_value == 0) {
    push(value);
    return;
  }
  ...
  node *newNode = new node(value, current->getptr());
  current->setptr(newNode);

} // end insertNode

// HOMEWORK TODO
// To find the last value in the linked list which is less than the new node’s
// value. i.e. find the node which is closest to the end (among those with value
// less than the given value)
//  i.e. where in the linked list should you insert. It approximates the correct
//  driving order for the driver.
...
// aren't any other nodes in the linkedlist which are less than and closer to
// the new value. Eg given nodes with values 3, 8, 11, 15, 17. Insert 14
// after 11. Insert 9 after 8. Insert 19 after 17. Insert 2 at the head.
int LinkedList::find_closest_less_than(int value) {
  ...
  while (current != nullptr) {
    ...
    if (currVal < value && currVal > closest) {
      closest = currVal;
    }
    current = current->getptr();
  }
...
bool LinkedList::link_list_empty() {
  return headptr == nullptr; // This return has to be updated
}
